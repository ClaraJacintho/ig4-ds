...
incidents$isScooter <- NA
for (row in 1:nrow(incidents)) {
  cc <- incidents[row, "CYLINDREE1"]
  sc <- "Motorcycle"
  if(cc<=125){
    sc <- "Scooter"
  }
  incidents[row, "isScooter"] <- sc
}
acidents$isScooter <- NA
for (row in 1:nrow(acidents)) {
  cc <- acidents[row, "CYLINDREE1"]
  sc <- "Motorcycle"
  if(cc<=125){
    sc <- "Scooter"
  }
  acidents[row, "isScooter"] <- sc
}

str(acidents$isScooter)
